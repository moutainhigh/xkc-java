...
public class BOpportunityServiceImpl extends ServiceImpl<BOpportunityMapper, BOpportunity> implements IBOpportunityService {
    ...
    @Autowired
	private BOpportunityMapper opportunityMapper;
...
//		up.setOpportunityID(unity.getOpportunitySource());
    ...
    @Override
    public int getBOpportunityAmount(String mobile) throws Exception {
        int amount = 0;
        QueryWrapper<BOpportunity> queryWrapper = new QueryWrapper<BOpportunity>();
        queryWrapper.eq("CustomerMobile", mobile).eq("Status", "5");
        amount = opportunityMapper.selectCount(queryWrapper);
        return amount;
    }
