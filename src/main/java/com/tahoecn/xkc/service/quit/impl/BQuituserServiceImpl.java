...
public class BQuituserServiceImpl extends ServiceImpl<BQuituserMapper, BQuituser> implements IBQuituserService {
    ...
    public IPage<Map<String, Object>> QuitUserSalesTeamList_Select(IPage page, Map<String, Object> map) {
        ...
        {
            sqlWhere.append(" AND CU.Name LIKE concat(concat('%', "+Name+"), '%')");
        }
        //用户手机号
        if (StringUtils.isNotEmpty(Mobile))
        {
            sqlWhere.append(" AND CU.Mobile LIKE concat(concat('%', "+Mobile+"), '%')");
        }
        //登录帐号
        if (StringUtils.isNotEmpty(UserName))
        {
            sqlWhere.append(" AND CU.UserName LIKE concat(concat('%', "+UserName+"), '%')");
        }
        //机构名称
        if (StringUtils.isNotEmpty(OrgName))
        {
            sqlWhere.append(" AND CO.OrgName LIKE concat(concat('%', "+UserName+"), '%')");
        }
    ...
    private void setSqlWhere(Map<String, Object> map) {
        ...
        if (StringUtils.isNotEmpty(Name))
        {
            sqlWhere.append(" AND A.EmployeeName LIKE concat(concat('%', "+Name+"), '%')");
        }
        //用户手机号
        if (StringUtils.isNotEmpty(Mobile))
        {
            sqlWhere.append(" AND A.Mobile LIKE concat(concat('%', "+Mobile+"), '%')");
        }
        //登录帐号
        if (StringUtils.isNotEmpty(UserName))
        {
            sqlWhere.append(" AND A.UserName LIKE concat(concat('%', "+UserName+"), '%')");
        }
        //机构名称
        if (StringUtils.isNotEmpty(TeamName))
        {
            sqlWhere.append(" AND SG.Name LIKE concat(concat('%', "+TeamName+"), '%')");
        }
        //项目ID
        if (StringUtils.isNotEmpty(ProjectID))
        {
            sqlWhere.append(" AND SG.ProjectID LIKE concat(concat('%', "+ProjectID+"), '%')");
        }
        //是否处理
        if (StringUtils.isNotEmpty(IsDispose))
        {
